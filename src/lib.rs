...
::paste::paste!{
    ...
    /// Initializes log4rs.
    fn pel_init_log4rs() {
        let pattern = ::log4rs::encode::pattern::PatternEncoder::new("{d(%Y-%m-%d %H:%M:%S)} {m}\n");
        let stdout = ::log4rs::append::console::ConsoleAppender::builder()
            .encoder(::std::boxed::Box::new(pattern))
            .build();

        let config = ::log4rs::Config::builder()
            .appender(::log4rs::config::Appender::builder().build("stdout", ::std::boxed::Box::new(stdout)))
            .build(::log4rs::config::Root::builder().appender("stdout").build(::log::LevelFilter::Trace))
            .unwrap();
